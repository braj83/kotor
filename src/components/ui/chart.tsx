...
function ChartStyle({ id, config }: { id: string; config: ChartConfig }) {
  const colorConfig = Object.entries(config).filter(([key]) => key !== 'x' && key !== 'y')
...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const value = !labelKey && typeof label === 'string' ? label : itemConfig?.label || key
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: any, key: string) {
  ...
  return configLabelKey in config ? config[configLabelKey as keyof ChartConfig] : undefined
}
